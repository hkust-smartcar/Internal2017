...
bool BTonReceiveInstruction(const Byte *data, const size_t size){
	...
	case 2:
		...
		break;
	case ' ':
		inAuto = 0; //press space key to enter manual mode
		stopMotor();
		setMotorDir(0);
		break;
	case 'w':
...
void about_centerline(const Byte* camPtr){
	int area_dif=0, x, white_area = 0, black_img_find=0;
	for(int line=0;line<60;line++){
		...
		for(int i=0; i<10; i++){
			for(int j=7; j>=0; j--){
				...
				if(!black_img_find) { // find the area diff on the top region of the img (decide the turning direction in a roundabout)
		...
		if(white_area<0.2*80) { // if almost no white track detected -> do not count this center line
			center[line] = 100;
			continue;
		}
		...
		if(in_turn && white_area>78){ // come to the end of roundabout
			if(in_turn==1){ //turn right
				sideL[line] = 40;
				sideR[line] = 80;
			} else{
		...
		if(line>10){ // cannot smooth the top 10 lines, otherwise array index will be negative
			if (center[line-10]<90 && center[line-9]<90 && abs(center[line-10]-center[line-9])>5){ // smooth center line
				for (int k=0;k<10;k++){
