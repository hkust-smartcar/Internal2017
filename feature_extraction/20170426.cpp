	...
	while(true){
		...
		if (System::Time()!=time_img){
			...
			if (cam->IsAvailable()&&time_img%50==0){
				...
				*/
				//===
				const int base_mid_x= (max(left_edge[0][0],0)+min(right_edge[0][0],WIDTH-1))/2;
				left_edge[0][0]=base_mid_x;right_edge[0][0]=base_mid_x;
				while(!get_pkbit(byte,--left_edge[0][0]-1,left_edge[0][1]));
				while(!get_pkbit(byte,++right_edge[0][0]+1,right_edge[0][1]));
...
/*
				...
				if(error>25){
					...
					//int left_black_sum=0,right_black_sum=0;
					int left_y=0,right_y=0;
					while(!get_pkbit(byte,left_edge[0][0],++left_y+1));
					while(!get_pkbit(byte,right_edge[0][0],++right_y+1));
					int different=right_y-left_y;
					if(different>0){
						sprintf(feature,"RIGHT");
					}
					else if(different<0){
